<?php
    ...
    // Logic to fetch records from database with core prepare statement techonlogy
    $select_querry = "SELECT * FROM signup_form_data";

    $result = mysqli_prepare($conn, $select_querry);
    mysqli_stmt_execute($result);
    mysqli_stmt_bind_result($result, $id, $user_name, $phone_number, $email_id, $new_password, $gender, $profile_pic);
    
    $result_arr = [];
    while($records = mysqli_stmt_fetch($result)){
    ...
    if(isset($_POST["delete_id"])){
        $delete_querry = "DELETE FROM signup_form_data WHERE id = ?";
        $result = mysqli_prepare($conn, $delete_querry);
        mysqli_stmt_bind_param($result, "i", $_POST["delete_id"]);
        mysqli_stmt_execute($result);
        if(!mysqli_stmt_error($result)){
            header("Location: ./index.php");
        }else{
            echo "<script>alert('Something went wrong try after some time !')</script>";
        }
