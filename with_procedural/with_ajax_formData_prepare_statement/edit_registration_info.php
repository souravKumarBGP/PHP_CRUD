<?php
    ...
    // Logic to select edit recored when user click on edit buttton of index.php page because save predefine value in input feild
    $edit_records_result = mysqli_prepare($conn, "SELECT * FROM signup_form_data WHERE id = ? ");
    mysqli_stmt_bind_param($edit_records_result, 'i', $_SESSION['edit_id'] );
    mysqli_stmt_execute($edit_records_result);
    mysqli_stmt_bind_result($edit_records_result, $id, $user_name, $phone_number, $email_id, $new_password, $gender, $profile_pic );

    $edit_records_arr = [];

    while(mysqli_stmt_fetch($edit_records_result)){
        $edit_records_arr = [
        ...
        foreach($form_data_array as $key => $value){
            ...
            if($value != ''){
                ...
                if(strlen($value) < 200){
                    ...
                    $email_exixt_result = mysqli_prepare($conn, "SELECT email_id FROM signup_form_data WHERE email_id = ?");
                    mysqli_stmt_bind_param($email_exixt_result, "s", $form_data_array['email_id']);
                    mysqli_stmt_execute($email_exixt_result);
                    mysqli_stmt_store_result($email_exixt_result);
                    
                    if(mysqli_stmt_num_rows($email_exixt_result) == 1){
    
                        // Logic to check check valid phone number or not
                        if(strlen($form_data_array["phone_number"]) == 10){
    
                            $image_path = "./uploded_img/".mysqli_real_escape_string($conn, $_FILES["profile_pic"]["name"]);
                            ...
                            $set_values = substr($set_values, 0 , -2); // remove exteral space and comma
                    ...
                    mysqli_stmt_free_result($email_exixt_result); // Free up the result set memory
                    mysqli_stmt_close($email_exixt_result); // Close the prepared statement
